...
spec =
  describe "combine" $ do
    ...
    it "should not modify list of criterna (name) per user story" $ do
      property (\(fs, ts) -> (combine fs ts) ^.. allCriteriaNames == fs ^.. allCriteriaNames)
    ...
    where
      ...
      allCriteriaNames = features.traverse.userStories.traverse.criteria.traverse.criteriaName
